...
#include "../include/jsonrpc-lean/util.h"
...
int main(int argc, char** argv)
{
  ...
  while ((res = read(fd, buffer.get(), size)) > 0) {
    if (encode) {
      auto str = jsonrpc::util::Base64Encode(buffer.get(), res);
      if (!verify) {
        std::cout << str << "\r\n";
      }
      else {
        auto binary = jsonrpc::util::Base64Decode(str);
        assert(binary.size() == static_cast<size_t>(res));
        assert(memcmp(buffer.get(), binary.data(), res) == 0);
      }
    }
    else {
      auto binary = jsonrpc::util::Base64Decode(
